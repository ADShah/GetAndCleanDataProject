...
The columns that represent the means and standard deviations of the measured variables, namely the features that end in *mean() or *std() are extracted. The script then calculates the means of these values for each subject/activity pair. 

The final tidy data set is constructed so that each row represents observations for subject/activity combination and and the observed values are the means of the measured mean/std from the original data set. The tidy data set is written to the working directory as UCI_HAR_Tidy_DataSet.txt

The format for the tidy data developed by the run_analysis.R script was developed based on the Table 3
